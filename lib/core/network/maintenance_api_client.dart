...
/// Vía Gateway: solo JWT (X-Tenant-Id/X-Driver-Id los inyecta el Gateway).
/// Directo (:8084): requiere X-Tenant-Id y X-Driver-Id.
class MaintenanceApiClient {
  MaintenanceApiClient({
    ...
    Dio? dio,
        ...
        _baseUrl = baseUrl ?? ApiConstants.maintenanceBaseUrl,
        _dio = dio ??
            Dio(
              BaseOptions(
  ...
  bool get _usesDirectService => _baseUrl.contains(':8084');

  Future<void> _onRequest(
    RequestOptions options,
    RequestInterceptorHandler handler,
  ) async {
    if (_usesDirectService) {
      ...
      if (tenantId == null || tenantId.isEmpty) {
        handler.reject(
          DioException(
            ...
            message: 'X-Tenant-Id obligatorio para mantenimiento directo.',
          ),
        );
        return;
      }
      ...
      if (driverId == null || driverId.isEmpty) {
        handler.reject(
          DioException(
            requestOptions: options,
            type: DioExceptionType.cancel,
            message: 'X-Driver-Id obligatorio para mantenimiento directo.',
          ),
        );
        return;
      }
    ...
    } else if (!_usesDirectService) {
      handler.reject(
        DioException(
          requestOptions: options,
          type: DioExceptionType.cancel,
          message: 'JWT obligatorio para mantenimiento vía Gateway.',
        ),
      );
      return;
    }
